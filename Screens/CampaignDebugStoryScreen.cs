...
/// <summary>Plays intro, interludes, and outro only; missions are skipped. For authoring dialogue.</summary>
public sealed class CampaignDebugStoryScreen : IGameScreen
{
    ...
    private int _beatPos;
    private int _lineIndex;
    private Texture2D _portrait;

    public CampaignDebugStoryScreen(Game1 game, LoadedCampaign campaign, string loadError)
    {
        _game = game;
        _campaign = campaign;
        _loadError = loadError;
        _beats = campaign != null
            ? GameFlowSchedule.DebugStorySkipMissionsBeatIndices(campaign.Missions.Count)
    ...
    {
        if (_campaign == null || _beatPos >= _beats.Length)
            return null;
        return _campaign.TryGetDialogueForBeat(_beats[_beatPos], out DialogueSegment seg) ? seg : null;
    }
    ...
    {
        if (_loadError != null || _campaign == null)
        {
            if (input.EscapePressed || input.BackPressed || input.ConfirmPressed)
            {
                ReleasePortrait();
                _game.ShowMainMenu();
            }
    ...
    public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
    {
        ...
        spriteBatch.DrawString(font, "Story debug (missions skipped)", new Vector2(48, 28), pal.TitleText, 0f,
            Vector2.Zero, TitleScale, SpriteEffects.None, 0f);

        if (_loadError != null)
        {
            spriteBatch.DrawString(font, "Could not load campaign:", new Vector2(48, 120), pal.PrimaryWhite, 0f,
                Vector2.Zero, MetaScale, SpriteEffects.None, 0f);
            spriteBatch.DrawString(font, _loadError, new Vector2(48, 168), pal.ButtonLabel, 0f, Vector2.Zero,
                BodyScale * 0.85f, SpriteEffects.None, 0f);
            spriteBatch.DrawString(font, "Confirm or Esc: back to menu", new Vector2(48, 600), pal.PrimaryWhite * 0.85f,
        ...
        if (seg == null || line == null)
        {
            spriteBatch.DrawString(font, "End of story debug.", new Vector2(48, 200), pal.PrimaryWhite, 0f,
                Vector2.Zero, MetaScale * 1.2f, SpriteEffects.None, 0f);
            return;
        }
        ...
        float ty = 120;
        string beatLabel = $"Beat {_beats[_beatPos]}  line {_lineIndex + 1}/{seg.Lines.Count}";
        ...
        UiChrome.DrawLabel(spriteBatch, font, "Next (Space / Enter / Click)", _nextButton, pal.ButtonLabel,
            NextLabelScale);
        spriteBatch.DrawString(font, "Esc: menu", new Vector2(48, 668), pal.PrimaryWhite * 0.65f, 0f, Vector2.Zero,
            HintScale, SpriteEffects.None, 0f);
    }
